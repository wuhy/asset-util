 ...
 * @return {string}
 */
function addDomainInfo(url, domain, file) {
    var isLocalPath = pathUtil.isLocalPath(url);
    if (pathUtil.isLocalPath(url) && _.isString(domain)) {
    ...
    if (_.isFunction(domain)) {
        return domain(url, isLocalPath, file);
    }
}
...
function replaceURL(replaceInfo, replacer, options) {
    ...
    url.forEach(function (item) {
        ...
        match = match.replace(
            regexp, '$1' + rewriteURL(item, replaceInfo.file, replacer, options)
        );
    });
